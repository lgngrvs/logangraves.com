...
def add_to_db(title, content):
    ...
    print(slug)
    connection.execute(f"INSERT INTO posts (title, slug, content) VALUES ('{title}', '{slug}', '{content}')")
    connection.commit()
    connection.close()
...
add_to_db(post1_title, post1_content)
add_to_db(post2_title, post2_content)